...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        numLeg = len(self.legalPositions)
        while numPar > 0:
            if numPar > numLeg:
                arr += self.legalPositions
                numPar -= numLeg
            else:
                arr += self.legalPositions[0:numPar]
                numPar = 0
        self.particles = arr
    ...
    def elapseTime(self, gameState):
        ...
        for i in range(self.numParticles):
            ...
            if oldPos in cache:
                self.particles[i] = cache[oldPos].sample()
            else:
                newPosDist = self.getPositionDistribution(gameState, oldPos)
                cache[oldPos] = newPosDist
